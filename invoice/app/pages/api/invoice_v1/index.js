...
export default async (req,res)=> {
    ...
    console.log('Process by Bank:', processByBank);
...
function getInvoiceFilename(invoiceNumber, invoiceName, processingBank, ext, folder = "") {
    ...
    // e.g. "001383_Invoice_NWIL3MHOCT25LRP1_L001383"
...
function generateExcel(invoice, invoiceNumber, folder) {
    console.log(`[${new Date().toISOString()}] Starting generateExcel for Invoice: ${invoiceNumber}`);
...
async function generatePDFFile(content, invoiceNumber, folder) {
  ...
  const outputPath = getInvoiceFilename(
    invoiceNumber,
    content.invoiceName,
    content.processingBank,
    "pdf",
    folder
  );
...
function readWorkbook(file) {
    ...
    const limitedLastRow = Math.min(dataRange.e.r, 1000); // 0-based index, 1000 rows max
